...
describe('Home template section CSS', () => {
  ...
  it('uses compact cards and compact filter controls', () => {
    ...
    const controlBarRule = getRule('.home-control-bar')
    const modeSwitchControlRule = getRule('.home-mode-switch__control')
    const modeSwitchButtonRule = getRule('.home-mode-switch__button')
    ...
    expect(modeSwitchControlRule).toContain('min-height: 36px;')
    expect(modeSwitchButtonRule).toContain('min-width: 65px;')
    ...
    expect(searchRule).toContain('flex: 0 0 121px;')
    expect(searchRule).toContain('min-width: 121px;')
    expect(searchRule).toContain('max-width: 137px;')
...
describe('Recommendation workbench CSS', () => {
  it('presents visual insight widgets as a 3.5-card horizontal rail', () => {
    ...
    const highlightRule = getRule('.recommendation-highlight')

    expect(workbenchRule).toContain('gap: 14px;')
    expect(workbenchRule).toContain('margin-top: 4px;')
    expect(signalStripRule).toContain('justify-content: flex-end;')
    expect(signalStripRule).toContain('align-items: center;')
    expect(appCss).not.toContain('.recommendation-workbench__attention-rail')
    expect(appCss).not.toContain('.recommendation-card--attention')
    ...
    expect(feedTagRule).toContain('right: 10px;')
    expect(loadMoreRule).toContain('height: 34px;')
    expect(loadMoreRule).toContain('background: transparent;')
    expect(highlightRule).toContain('border-color: rgba(8, 143, 182, 0.58);')
    expect(highlightRule).toContain('box-shadow: 0 0 0 2px rgba(8, 143, 182, 0.12), var(--shadow-panel);')
  })

  it('styles recommendation detail routes as focused desktop panels', () => {
    const detailRule = getRule('.recommendation-detail-page')
    const heroRule = getRule('.recommendation-detail-hero')
    const gridRule = getRule('.recommendation-detail-grid')

    expect(detailRule).toContain('height: calc(100svh - 48px);')
    expect(detailRule).toContain('overflow-y: auto;')
    expect(heroRule).toContain('border: 1px solid var(--line);')
    expect(heroRule).toContain('border-radius: 10px;')
