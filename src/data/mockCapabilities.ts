...
export const capabilityEntries: MockCapabilityEntry[] = [
  ...
  },
  {
    ...
    description:
      '承接酶库设计交接，完成实验订单提交、样本准备、assay 执行同步、结果包回收和操作索引归档。',
    status: 'active',
    source: 'created',
    owner: '实验运营',
    version: 'v1.0',
    ...
    sections: [
      {
        title: '执行边界',
        items: [
          '订单范围固定为 ENZ-LIB-20260602-048 的 48 个候选酶与指定对照',
          '实验执行仅覆盖样本准备、assay 运行同步、结果包回收和归档',
        ],
      },
      {
        title: '审批与异常',
        items: [
          'BM-LAB-ENZ-20260602-001 提交前必须通过正式 Approval Gate',
          '异常孔保留原始读数、板图位置和人工复核记录，不做自动剔除',
        ],
      },
      {
        title: '结果交付',
        items: [
          '回收 Enzyme_Experiment_Result_Package.xlsx，包含原始读数、板图映射、QC/异常表和 schema 校验结果',
          '归档 Experiment Task、运行日志、样本追踪和审批记录索引',
        ],
      },
    ],
    recentActivity: [
      '提交实验订单 BM-LAB-ENZ-20260602-001 并记录 Approval Gate',
      '回收 Enzyme_Experiment_Result_Package.xlsx 并完成 schema 校验',
      '归档 Experiment Task、运行日志、异常复核记录和样本追踪索引',
    ],
    steps: [
      '读取设计交接',
      '确认订单边界',
    ...
    dag: {
      nodes: [
        {
          id: 'read-design-handoff',
          kind: 'input',
          subtype: 'data',
          title: '读取设计交接',
          shortTitle: '读取设计交接',
          description: '读取 ENZ-LIB-20260602-048 设计交接包，确认 parent enzyme、候选变体范围和禁止自动动作。',
          inputs: ['ENZ-LIB-20260602-048', 'parent enzyme', '候选变体范围'],
          outputs: ['设计交接摘要', '禁止自动动作清单'],
          prerequisites: ['设计交接包可读取'],
          resources: [{ kind: 'data-system', name: 'Design Handoff Registry' }],
          layout: { row: 0, column: 0 },
        },
        {
          id: 'confirm-order-boundary',
          kind: 'operation',
          subtype: 'data',
          title: '确认订单边界',
          shortTitle: '订单边界',
          description: '把实验执行范围固定为 48 个候选酶、指定对照、SOP v3 和本轮订单目标。',
          inputs: ['设计交接摘要', '实验约束'],
          outputs: ['订单边界记录', '禁止外推范围'],
          prerequisites: ['设计交接已读取'],
          resources: [{ kind: 'lab-system', name: 'Order Boundary Console' }],
          layout: { row: 1, column: 0 },
        },
        {
          id: 'freeze-sample-scope',
          kind: 'operation',
          subtype: 'sample',
          title: '固化样本范围',
          shortTitle: '样本范围',
          description: '固化 48 个候选酶、对照、分装计划、样本批次和孔板关联记录。',
          inputs: ['订单边界记录', '样本库存'],
          outputs: ['样本范围清单', '孔板关联基线'],
          prerequisites: ['订单边界已确认'],
          resources: [{ kind: 'sample-storage', name: 'Enzyme Sample Storage' }],
          layout: { row: 2, column: 0 },
        },
        {
          id: 'freeze-readout-panel',
          kind: 'operation',
          subtype: 'lab-operation',
          title: '固化读数面板',
          shortTitle: '读数面板',
          description: '固定读数面板、SOP v3、底物批次 QC 要求、设备和实验线路。',
          inputs: ['样本范围清单', '读数面板', 'SOP v3', '底物批次 QC'],
          outputs: ['读数面板基线', '执行约束清单'],
          prerequisites: ['样本范围已固化'],
          resources: [{ kind: 'lab-system', name: 'Assay Planning Console' }],
          layout: { row: 3, column: 0 },
        },
        {
          id: 'generate-plate-map',
          kind: 'operation',
          subtype: 'sample',
          title: '生成板图',
          shortTitle: '板图',
          description: '为 48 个候选酶和对照生成可追溯板图，绑定样本批次、孔位和读数面板。',
          inputs: ['孔板关联基线', '读数面板基线'],
          outputs: ['板图映射', '孔位追踪表'],
          prerequisites: ['读数面板已固化'],
          resources: [{ kind: 'lab-system', name: 'Plate Map Builder' }],
          layout: { row: 4, column: 0 },
        },
        {
          id: 'check-sample-inventory-plate',
          kind: 'operation',
          subtype: 'sample',
          title: '检查样本库存与孔板关联',
          shortTitle: '库存孔板',
          description: '检查样本库存、分装计划、样本批次和孔板关联是否与板图一致。',
          inputs: ['样本范围清单', '板图映射'],
          outputs: ['库存孔板检查记录', '待复核差异'],
          prerequisites: ['板图已生成'],
          resources: [{ kind: 'sample-storage', name: 'Inventory Ledger' }],
          layout: { row: 5, column: 0 },
        },
        {
          id: 'check-material-sop-device-route',
          kind: 'operation',
          subtype: 'lab-operation',
          title: '检查物料/SOP/设备/线路',
          shortTitle: '物料线路',
          description: '检查底物批次 QC、SOP v3、设备可用性和实验线路，形成提交前执行条件记录。',
          inputs: ['执行约束清单', '库存孔板检查记录'],
          outputs: ['执行条件记录', '设备线路确认'],
          prerequisites: ['库存与孔板关联已检查'],
          resources: [
            { kind: 'device', name: 'Plate Reader' },
            { kind: 'lab-system', name: 'SOP Registry' },
          ],
          layout: { row: 6, column: 0 },
        },
        {
          id: 'order-approval-gate',
          kind: 'human-gate',
          subtype: 'data',
          title: '订单提交审批',
          shortTitle: '提交审批',
          description: '通过正式 Approval Gate 审批 BM-LAB-ENZ-20260602-001，确认可提交实验订单。',
          inputs: ['订单边界记录', '执行条件记录', '设备线路确认'],
          outputs: ['已提交实验订单 BM-LAB-ENZ-20260602-001', '审批记录'],
          prerequisites: ['执行条件记录完整', '正式审批人已指定'],
          control: {
            kind: 'approval-gate',
            summary: '订单提交必须通过正式 Approval Gate。',
          },
          resources: [{ kind: 'lab-system', name: 'Approval Console' }],
          layout: { row: 7, column: 0 },
        },
        {
          id: 'create-experiment-task',
          kind: 'operation',
          subtype: 'lab-operation',
          title: '创建 Experiment Task',
          shortTitle: 'Experiment Task',
          description: '根据已提交订单创建 Experiment Task，绑定订单、样本、板图、SOP 和执行线路。',
          inputs: ['已提交实验订单 BM-LAB-ENZ-20260602-001', '审批记录', '板图映射'],
          outputs: ['Experiment Task', '执行记录索引草稿'],
          prerequisites: ['订单提交审批已通过'],
          resources: [{ kind: 'lab-system', name: 'Experiment Task Console' }],
        ...
        },
        {
          ...
          title: '同步样本准备',
          shortTitle: '样本准备',
          description: '同步样本准备状态、分装记录、样本追踪和孔板实际占用情况。',
          inputs: ['Experiment Task', '样本范围清单', '孔位追踪表'],
          outputs: ['样本准备日志', '样本追踪记录'],
          prerequisites: ['Experiment Task 已创建'],
          resources: [{ kind: 'lab-system', name: 'Sample Prep Tracker' }],
          layout: { row: 9, column: 0 },
        },
        {
          id: 'sync-assay-execution',
          kind: 'operation',
          subtype: 'lab-operation',
          title: '同步 assay 执行',
          shortTitle: 'assay 执行',
          description: '同步 assay 运行日志、设备读数状态、SOP v3 执行记录和实验线路状态。',
          inputs: ['样本准备日志', '读数面板基线', '设备线路确认'],
          outputs: ['运行日志', '原始读数索引'],
          prerequisites: ['样本准备已同步'],
          resources: [
            { kind: 'device', name: 'Plate Reader' },
            { kind: 'data-system', name: 'Run Log Index' },
          ],
          layout: { row: 10, column: 0 },
        },
        {
          id: 'recover-result-package',
          kind: 'operation',
          subtype: 'data',
          title: '回收结果包',
          shortTitle: '结果包',
          description: '回收 Enzyme_Experiment_Result_Package.xlsx 及其原始读数、板图映射、QC/异常表。',
          inputs: ['运行日志', '原始读数索引', '板图映射'],
          outputs: ['Enzyme_Experiment_Result_Package.xlsx', '结果包清单'],
          prerequisites: ['assay 执行已同步'],
          resources: [{ kind: 'data-system', name: 'Result Package Store' }],
          layout: { row: 11, column: 0 },
        },
        {
          id: 'record-anomaly-events',
          kind: 'operation',
          subtype: 'data',
          title: '记录异常事件',
          shortTitle: '异常事件',
          description: '记录异常孔、异常事件和人工复核入口，保留所有原始读数，不做自动剔除。',
          inputs: ['结果包清单', '原始读数索引'],
          outputs: ['异常复核记录', '原始读数保留声明'],
          prerequisites: ['结果包已回收'],
          resources: [{ kind: 'data-system', name: 'Anomaly Review Queue' }],
          layout: { row: 12, column: 0 },
        },
        {
          id: 'validate-result-package-schema',
          kind: 'qc-decision',
          subtype: 'data',
          title: '校验结果包 schema',
          shortTitle: 'schema 校验',
          description: '校验 Enzyme_Experiment_Result_Package.xlsx 的原始读数、板图映射、QC/异常表和 schema 校验结果。',
          inputs: ['Enzyme_Experiment_Result_Package.xlsx', '异常复核记录'],
          outputs: ['schema 校验结果', '可归档结果包'],
          prerequisites: ['异常事件已记录'],
          control: {
            kind: 'preset-qc-check',
            summary: '只校验结果包 schema 和完整性，不自动剔除异常孔。',
          },
          resources: [{ kind: 'data-system', name: 'Result Schema Validator' }],
          layout: { row: 13, column: 0 },
        },
        {
          id: 'archive-operation-index',
          kind: 'output',
          subtype: 'report',
          title: '归档操作索引',
          shortTitle: '归档操作索引',
          description: '归档订单审批、Experiment Task、运行日志、异常复核记录、样本追踪和结果包索引。',
          inputs: ['schema 校验结果', '可归档结果包', '执行记录索引草稿'],
          outputs: [
            '执行记录索引',
            'Enzyme_Experiment_Result_Package.xlsx',
            'BM-LAB-ENZ-20260602-001 归档记录',
          ],
          prerequisites: ['结果包 schema 校验完成'],
          resources: [
            { kind: 'data-system', name: 'Experiment Record Index' },
            { kind: 'data-system', name: 'Archive Registry' },
          ],
          layout: { row: 14, column: 0 },
        },
      ],
      edges: [
        { from: 'read-design-handoff', to: 'confirm-order-boundary', label: '交接' },
        { from: 'confirm-order-boundary', to: 'freeze-sample-scope', label: '边界' },
        { from: 'freeze-sample-scope', to: 'freeze-readout-panel', label: '样本' },
        { from: 'freeze-readout-panel', to: 'generate-plate-map', label: '面板' },
        { from: 'generate-plate-map', to: 'check-sample-inventory-plate', label: '板图' },
        { from: 'check-sample-inventory-plate', to: 'check-material-sop-device-route', label: '库存' },
        { from: 'check-material-sop-device-route', to: 'order-approval-gate', label: '待审批' },
        { from: 'order-approval-gate', to: 'create-experiment-task', label: '通过' },
        { from: 'create-experiment-task', to: 'sync-sample-prep', label: '任务' },
        { from: 'sync-sample-prep', to: 'sync-assay-execution', label: '样本准备' },
        { from: 'sync-assay-execution', to: 'recover-result-package', label: '原始读数' },
        { from: 'recover-result-package', to: 'record-anomaly-events', label: '结果包' },
        { from: 'record-anomaly-events', to: 'validate-result-package-schema', label: '异常记录' },
        { from: 'validate-result-package-schema', to: 'archive-operation-index', label: 'schema 通过' },
      ],
    },
  },
  {
